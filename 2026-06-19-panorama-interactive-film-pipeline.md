...
1. 执行顺序以 §0.1 为准：**先做 v0 平面 fixture/demo 最短闭环**（fixture manifest → `panorama-player` 平面模式 → Vercel 链接），再做 v0.5 技术 spike，**之后**才完成前 6 步创作端与节点画布的数据模型与页面流程，最后把图片、视频、音频接到节点资产槽里。本节其余结论描述的是 v1 及以后的完整形态，不代表第一步就要做完整创作流。
...
5. 主项目 `bunx tsc --noEmit` 的既有类型债作为独立任务单独治理，不阻塞 MVP v0 最短闭环验证；只需先修复与 v0/v0.5 直接相关的类型错误。全仓类型债清零是发布质量门禁的目标，但不是 v0 的前置条件。
...
| 层级 | 目标 | 做什么 | 明确不做 |
|---|---|---|---|
| MVP v0 最短闭环 | 用现有 demo/fixture 证明“可玩链接”真实成立 | 现有故事或最小 `.dfstory` -> `player-manifest` 平面图模式 -> `panorama-player` -> Vercel URL | 不重写完整前 6 步，不接自进化，不接视频生成，不强制全景 |
...
因此，`EvoMap/Evolver`、玩家行为回流、用户自有部署、对象存储生产化和完整前 6 步重构都保留为路线图能力；MVP v0 只验证最短可玩链路。
...
| RULE-324 | MVP v0 必须先用现有 demo/fixture 跑通 `.dfstory/player-manifest -> panorama-player -> Vercel URL` 最短闭环，再启动完整前 6 步重构。 |
...
| RULE-335 | 创作端状态必须有单一真相源。进入 v1 前要明确决定：是扩展现有 `useNarrativeStore` 数据，还是新立 `useCreationFlowStore`；不允许 `useCreationFlowStore`、`useNarrativeStore`、`usePipelineStore` 三个 store 长期并存且各自持有重叠的故事状态，否则 stale 传播与同步会成为长期 bug 来源。 |
| RULE-336 | `local-codex-worker` / 本机 Node 进程部署仅用于 v0 demo 验证最短闭环，不得作为长期或对外方案。"任何人点击即部署并返回链接"的产品能力，必须在进入 v1/对外前迁移到常驻服务端 Worker + 服务端持有的 Vercel Token；浏览器与本机登录态不得成为部署链路的依赖。 |
| RULE-337 | 全景资产来源默认走「用户自制上传」（图片/视频均为 2:1 equirectangular）。**若启用全景图生成**，图片 prompt 必须由系统自动追加固定全景后缀，不依赖创作者手写：`360 degree equirectangular panorama, seamless 2:1, 4096x2048, viewer standing at center, first-person perspective, no text, no logo, no watermark`。该后缀在 `src/lib/asset-pipeline/prompt.ts` 中作为常量维护，全景节点生成时强制拼接；平面节点不追加。生成结果仍须经全景 spike 验收（RULE-325），不通过不批量启用。 |
...
> ⚠️ 单一真相源约束（RULE-335）：上图把 `useCreationFlowStore` 设为 Step 1-6 真相源，与现有 `useNarrativeStore`、`usePipelineStore` 并存。这是为了 v1 重写创作端做的设计，但**三个 store 各自持有重叠的故事状态会成为同步/stale bug 的长期来源**。进入 v1 前必须正面决策：要么扩展现有 `useNarrativeStore` 承载 Step 1-6 中间态，要么明确以 `useCreationFlowStore` 为唯一源、其余 store 仅作派生缓存。不要让三个并行真相源长期共存。该决策不影响 v0（v0 不动创作端）。
...
flowchart TD
  ...
  M --> IMG["image panorama"]
  M --> VID["video panorama"]
...
| `mediaSlots.image` | 必需 | 全景图槽位；缺失可 fallback，但发布 warning |
...
| `image_ready` | 有全景图 URL 或明确 fallback | 可发布图文互动版 |
...
| 每个项目生成的轻量全景图 | `.chasedream-build/<projectId>/player/public/assets/nodes/<nodeId>/panorama.jpg` | OSS/R2/Vercel Blob + CDN | MVP 可随 Vercel 静态包部署 |
...
```json
{
  ...
  "imageUrl": "/assets/nodes/ep01-s01-rain-night/panorama.jpg",
  "voice": {
...
{
  "nodeId": "ep01-s01-rain-night",
  "imageUrl": "https://cdn.example.com/projects/p1/nodes/ep01-s01-rain-night/panorama.jpg",
  "videoUrl": "https://cdn.example.com/projects/p1/nodes/ep01-s01-rain-night/panorama.mp4",
  "voice": {
...
MVP 推荐 Deploy Service 创建部署任务，再由 local-codex-worker 或 Node Worker 调 Vercel API/CLI，不在浏览器执行部署。
...
> ⚠️ 执行器边界（RULE-336）：`local-codex-worker` / 本机 Node 进程**只用于 v0 demo 验证最短闭环**，它依赖某台开发机开机并保持 Vercel 登录态，**不是产品能力**。要真正做到"任何人在前端点按钮→出链接"，正式 MVP 这步（常驻 `node-worker` + 服务端持有的 Vercel Token）不能省略，也不能让 v0 的本机方式悄悄沉淀为长期方案。
...
```text
用户点击“部署到 Vercel”
  -> POST /api/panorama/deploy { projectId, environment, deploymentTargetId? }
  -> 服务端创建 queued deploy record
...
sequenceDiagram
  ...
  UI->>DS: POST /api/panorama/deploy { projectId, dfstoryId, deploymentTargetId? }
  DS->>DS: resolve deployment target
  DS->>DS: create queued deploy record
  DS-->>UI: { deployId, status: "queued" }
  WK->>DS: claim deploy job
  WK->>PK: package(dfstory, assets, config)
...
MVP v0 不重写完整前 6 步，也不接自进化。它只验证“现有内容能否变成可玩的公网链接”。
...
| 部署 | 返回一个公网 URL，打开后能玩 |
| 记录 | 保存 deploy URL、deployment id、构建方式和使用的 manifest 版本 |
...
9. 上传自制全景资产时，系统只负责校验、绑定和播放，不调用 Image 2 或视频生成 API。不符合 2:1 的文件**不静默自动降级**：必须阻止其绑定为 `panorama-image`/`panorama-video`，并提示用户改为平面资产或重新上传合格全景文件（与 EDGE-037 一致），避免用户误以为绑定的是全景。
10. 全景资产来源默认走「用户自制上传」。后续若开放全景图**生成**，图片 prompt 必须追加以下固定后缀（见 RULE-337）：`360 degree equirectangular panorama, seamless 2:1, 4096x2048, viewer standing at center, first-person perspective, no text, no logo, no watermark`。
...
1. 用同一份最小 `player-manifest.json` 分别跑 A/B 两种路径。
2. 记录从点击发布到可玩 URL 的耗时。
3. 验证手机端打开速度、缓存更新、manifest 404、资产 404、跨域失败。
4. 如果 B 通过，v1 以后默认采用“播放器部署一次 + gameId 拉 manifest”；A 保留给每游戏独立站点或客户自定义域名。

### 5.13 播放器交互 Spike

播放器必须在 v0.5 验证三类浏览器行为：
...
export interface PlayableChoiceDraft {
  ...
  caption?: string;
  next: string;
  // 扁平数值模型（§13「数值模型拍平」）：effect/condition 均针对扁平命名变量 key
  effect?: {
    vars?: Record<string, number>;   // 变量 key -> 增量
    flags?: string[];
    memories?: string[];
  };
  condition?: {
    flags?: string[];
    missingFlags?: string[];
    minVars?: Record<string, number>;  // 变量 key -> 最小值（含）
    maxVars?: Record<string, number>;  // 变量 key -> 最大值（含）
  };
}
...
export interface PanoramaPlayerManifest {
  ...
  // 扁平数值模型（§13「数值模型拍平」）：用单一变量表替代两层 metrics/globals
...
// 扁平命名变量；与 dfstory GameVariable 一一对应
export interface PlayerVariable {
  key: string;          // 唯一变量 key，如 "stealth_score" 或 "qi_spark"
  label: string;
  initial: number;
  min?: number;         // 默认 0
  max?: number;         // 默认 100
  scope?: "global" | "character";  // 仅 UI 分组用，不影响引擎计算
  characterId?: string;            // scope=character 时归属角色（仅 UI）
}
```
...
// 扁平数值模型（§13）：运行时 effect/condition 都针对扁平变量 key
...
| API-008 | `deployPanoramaProject(projectId, environment, deploymentTargetId?)` | projectId、环境、可选部署目标 | queued deploy record | 创建一键部署任务；MVP 默认使用 platform-hosted |
...
| API-012 | `packagePanoramaProject(projectId)` | dfstory、assetManifest | player manifest + build dir | 封装模板 |
...
1. `POST /api/panorama/deploy` 在 MVP 可以不传 `deploymentTargetId`，服务端自动选择 `isDefault = true` 的 `platform-hosted` 目标。
2. `POST /api/panorama/deploy` 只创建 `queued` deploy record；真正的 packager、build、Vercel deploy、轮询由 `local-codex-worker` 或独立 `node-worker` 执行。
3. 后续商业化阶段，前端只能传 `deploymentTargetId`，不得传 Vercel Token 明文。
4. `deployment-settings` 只返回脱敏后的 team/project/displayName/enabled 状态，不能返回 `credentialRef` 对应的真实密钥值。
5. 如果部署目标未配置或不可用，`deployPanoramaProject` 返回 `status = "failed"` 的 deploy record，并给出可读错误原因。
6. `exportDfStory` 必须校验 `NodeCanvasState.status = "confirmed"`；未确认时只能返回 draft export，不能进入正式部署。
...
```text
projects/<projectId>/nodes/<nodeId>/panorama.jpg
projects/<projectId>/nodes/<nodeId>/panorama.mp4
...
| 全景图 | image prompt、状态、预览、URL、来源 | 生成、重新生成、手动上传、粘贴 URL、替换、解除绑定、接受 fallback |
...
| `src/lib/asset-pipeline/prompt.ts` | 全景图、视频、TTS prompt 生成 |
...
- [ ] 准备一份最小 fixture，可以来自现有 `packages/panorama-player/src/data/story.ts` 或手写 3 个节点。
- [ ] 将 fixture 转成最小 `PanoramaPlayerManifest`，包含 `startNodeId`、nodes、choices、characters 和 assets。
...
- [ ] 用当前 Vercel 配置部署一次，返回公网 URL。
- [ ] 记录 deploy URL、deployment id、manifest 版本和部署耗时。
- [ ] 这个阶段不接完整 Step 1-6、不接自进化、不接视频生成，也不要求全景图片或全景视频。
...
- [ ] `panorama-deploy.ts` 只创建 `queued` deploy record，并返回 `deployId` 给 UI 轮询。
...
- [ ] Deploy Worker 调 `packages/packager` 生成临时 player。
...
| TEST-117 | MVP v0 最短闭环 | fixture -> player-manifest -> panorama-player 本地试玩 -> Vercel URL |
...
1. 准备 3 个节点的 fixture，生成最小 `player-manifest.json`。
2. 启动 `packages/panorama-player`，确认起始节点、选择跳转、结局页、fallback 资产都可用。
3. 确认第一个 demo 的所有节点都使用 `visualMode = "flat"`、`projection = "planar"`。
4. 用当前本机/Codex Vercel 配置部署一次，确认返回本次游戏 URL。
5. 用平面图 fixture 完成一次本地试玩和部署，确认不依赖全景图 spike。
6. 平面链路跑通后，上传一张自制 2:1 equirectangular 图片或一段 2:1 全景视频，确认它能绑定为 panorama 资产且不调用生成 API。
7. 上传一个 16:9 视频并选择全景模式，确认系统拦截 panorama 绑定并建议作为平面视频使用。
8. 用 2-3 个候选图片 provider 生成 2:1 样图，放入播放器球面场景检查接缝、极点、人物形变和移动端观感。
9. 同一份 manifest 分别跑 build-per-game 与 runtime-manifest spike，记录发布耗时、缓存更新、404 和 CORS 表现。
10. 在桌面 Chrome/Safari、iOS Safari、Android Chrome 上验证“开始体验”后音频可播、TTS 不叠加、陀螺仪授权失败可降级。
11. 根据 spike 结果决定 v1 是否开放全景节点、是否采用 runtime-manifest；平面图模式继续作为默认发布路径。
...
12. 批量生成图片或 TTS 前确认项目级成本预算弹窗存在，并可因超预算阻塞任务。
13. 节点画布 confirmed 后导出可发布 `.dfstory`，运行 `validateDfStory` 和跨 schema 一致性校验。
14. 通过 packager 生成 player manifest。
15. MVP 模式下把轻量资产复制进构建目录并启动 `packages/panorama-player`，确认可进入起始节点、点击选择、切换节点。
16. 在封装与部署页确认默认目标为“平台托管 Vercel”，用户无需填写账号。
17. 点击“部署到 Vercel”，确认 API 返回 queued deploy record，local-codex-worker 或 Node Worker 完成后返回本次游戏的可玩 URL，而不是默认覆盖 `https://meinubeibaoweila.vercel.app/`。
18. 打开公开链接，确认首次进入显示匿名数据提示；继续游玩后完成至少 3 条路径，确认匿名 `choice_selected`、`session_completed` 可上报。
19. 通关后提交评分、标签和可选文本反馈。
20. 回到 Studio 自进化报告页，按 deployment 生成报告。
21. 生成候选改版建议，确认所有候选默认为 `needs_review`。
22. 接受 1 条候选建议，确认不会自动修改线上部署版本。
...
| 决策 | 原因 | 采用方案 | 落入文档位置 |
|---|---|---|---|
| MVP 执行层级 | 一开始做全量 Step 1-6、资产、部署和自进化会把风险混在一起 | 先跑 v0 最短闭环，再做 v0.5 技术 spike，最后进入 v1 完整链路 | 0.1、5.10、Phase -1、TEST-117 |
...
| 数值模型拍平 | 现有 `GameVariable` 是扁平变量（无作用域、不绑定角色），而播放器 manifest 的两层 `metrics(按角色)+globals` 与 `choice.effect.relationships` 在扁平 dfstory 里没有来源，会造成映射歧义和往返字段漂移 | 播放器默认采用**扁平数值模型**：所有数值都是命名变量，与 dfstory `GameVariable` 一一对应，用统一数值面板渲染；`choice.effect` 简化为 `Record<string, number>`。仅当 Step 3 显式给变量打 `scope=character` 标记时，才在该变量上升级为按角色分组。这样 packager 无需"分类规则"，也避免富节点→拆分→重拼的丢失 | 6.9/6.11（manifest `metrics/globals` 调整为扁平变量表 + 可选 scope）、6.6.1（`PlayableChoiceDraft.effect` 简化）、Phase 6/7（player 泛化与 packager 映射）、TEST-006/005（按扁平变量验证） |
| 全景资产来源 | 全景图生成（equirectangular）可行性未验证，且第一阶段优先平面图；需要明确全景资产从哪来 | 全景资产**默认走「用户自制上传」**（2:1 equirectangular，图片/视频）；上传不合格（非 2:1）阻止绑定为全景并提示，不静默降级。若后续启用全景生成，图片 prompt 由系统强制追加固定全景后缀（见 RULE-337），生成结果仍须经全景 spike 验收 | RULE-334/337、5.10.1（第 9/10 条）、EDGE-037、`src/lib/asset-pipeline/prompt.ts` 常量 |
...
| CAP-012 | 播放器封装 | PAGE-012 | BTN-022 | ACT-012 Packager 转换 | DATA-010 PlayerManifest | API-012 | RULE-305 | TEST-005 |
| CAP-013 | Vercel 部署 | PAGE-012 | BTN-023 | ACT-013 创建部署 | DATA-011 DeployRecord | API-008 | RULE-306 | TEST-106/208 |
...
| CAP-022 | MVP v0 最短闭环 | PAGE-012/PAGE-016 | BTN-022/023 | ACT-022 fixture manifest 本地试玩并部署 | DATA-010/DATA-011 | API-008/012/013 | RULE-324 | TEST-117 |
...
| CAP-027 | 草稿兜底与 schema 一致性 | PAGE-012/PAGE-013 | BTN-042 | ACT-027 导出快照并校验三套 manifest 引用 | DATA-025 SchemaConsistencyReport | API-006/012 | RULE-330/331 | TEST-011/121/122 |
...
| FLOW-010 | 封装 | DATA-008 + DATA-009 -> packager -> player manifest | DATA-010 | manifest 校验失败 |
| FLOW-011 | 部署 | DATA-010 -> resolve deployment target -> queued DeployRecord -> Worker build -> Vercel -> poll | DATA-011 live | failed record + retry |
...
| FLOW-016 | 模板注入 | DATA-012 + DATA-009 -> DATA-010 -> player generated data | 可构建模板 | 缺 startNodeId 阻塞 |
...
| FLOW-021 | MVP v0 最短闭环 | fixture -> DATA-010 -> 本地试玩 -> Vercel deploy | DATA-011 live URL | 构建或部署失败则回退到 fixture/manifest 修复 |
...
| FLOW-026 | schema 一致性校验 | DATA-008 + DATA-009 + DATA-010 -> consistency validator | DATA-025 pass/fail | fail 时阻塞部署并列出字段 |
...
| DATA-008 | `DfStory` | dfstory-mapper | API-006 | packager、外部导出 |
| DATA-009 | `AssetManifest` | asset pipeline | API-007 | packager、Studio |
...
| API-008 | PAGE-012 | deploy service | DATA-010 + DATA-020 | queued DATA-011 | TEST-208/113/115 |
...
| API-012 | PAGE-012 | packager service | DATA-008 + DATA-009 | DATA-010 | TEST-005 |
...
| PAGE-012 | 检查清单、封装状态、部署目标、部署状态、schema 一致性结果 | BTN-022/023/042 | DATA-008/010/011/020/025 | Vercel READY |
...
| NODE-008 | `choices[].effect` | Step 3 状态设计 + Step 5 互动设计草案 | PAGE-005/PAGE-007/PAGE-013 | 数值变化 |
...
| hardcoded metrics | `playerManifest.variables`（扁平变量表，§13） | packager + player runtime |
...
- **v0 fixture deploy**：只校验 `player-manifest + asset manifest`，**不要求** Step 1-6 / 节点画布 / dfstory。用于最短闭环与自制资产验证。
- **v1 formal deploy**：走完整创作链路门禁（Step 1-6 confirmed → 节点画布 confirmed → dfstory 校验 → asset manifest）。

```mermaid
flowchart TD
  D0["用户点击部署到 Vercel"] --> M{"部署模式"}
  M -->|v0 fixture| F1["校验 player-manifest（startNode/节点/choices.next）"]
  F1 --> F2["校验 asset manifest（引用可达）"]
  F2 --> T0
  M -->|v1 formal| D1["校验 Step 1-6 confirmed"]
  D1 --> D2["校验节点画布 confirmed"]
  D2 --> D3["校验 dfstory"]
  D3 --> D4["校验 asset manifest"]
  D4 --> T0["读取 deploymentTargetId"]
  T0 --> T1{"是否传入 target?"}
  T1 -->|否| T2["使用 platform-hosted 默认目标"]
  T1 -->|是| T3["校验 user-owned/platform target"]
  T2 --> J0["创建 queued DeployRecord"]
  T3 --> J0
  J0 --> J1["local-codex-worker / node-worker 领取任务"]
  J1 --> D5{"MVP bundled assets?"}
...
| 阻塞编号 | 适用模式 | 条件 | 返回给用户 |
|---|---|---|---|
| BLOCK-001 | 仅 v1 formal | Step 6 未审核通过 | “还有未审核剧本，不能发布正式版” |
| BLOCK-002 | 仅 v1 formal | 节点画布未确认 | “请先确认节点画布里的文本、选项、资产槽位” |
| BLOCK-003 | v0 + v1 | `startNodeId` 不存在（manifest 起始节点缺失） | “起始节点缺失，请检查 manifest/节点画布” |
| BLOCK-004 | v0 + v1 | `choices.next` 指向不存在节点 | “存在断链选择，请修复 manifest/节点画布” |
| BLOCK-005 | v0 + v1 | MVP 静态包超出阈值 | “资产包过大，请切换对象存储/CDN 模式” |
| BLOCK-006 | v0 + v1 | Packager manifest 校验失败 | “播放器封装失败，查看字段错误” |
| BLOCK-007 | v0 + v1 | Vercel token 缺失 | “部署服务未配置 Vercel Token” |
| BLOCK-008 | v0 + v1 | Vercel 返回 failed | “部署失败，保留日志并可重试” |
| BLOCK-009 | v0 + v1 | 默认 `platform-hosted` target 不存在 | “平台托管部署目标未初始化” |
| BLOCK-010 | 仅 v1（user-owned） | 用户自有 target 凭证失效 | “请在部署设置中重新验证 Vercel 授权” |
| BLOCK-011 | v0 + v1 | Deploy Worker 未运行 | “部署执行器未启动，请启动本机/Codex worker 或 Node Worker” |
...
```mermaid
flowchart TD
  V001["v0: 现有 demo/fixture"] --> V002["生成最小 player-manifest: flat/planar"]
  V002 --> V003["panorama-player 平面图本地试玩"]
  V003 --> V004["Vercel 部署返回 URL"]
  V004 --> V005["v1 默认平面图创作链路"]
  V004 --> U001["上传自制 2:1 全景图片/视频"]
  U001 --> U002{"2:1 校验通过?"}
  U002 -->|是| U003["绑定 panorama-image / panorama-video"]
  U002 -->|否| U004["降级 flat-image / flat-video"]
  U003 --> V101["v1 可选开放全景节点"]
  U004 --> V005
  V004 --> S001["v0.5: 全景图 spike"]
  V004 --> S002["v0.5: 发布架构 spike"]
  V004 --> S003["v0.5: 播放器交互 spike"]
...
| Spike/验证项 | 输入 | 操作 | 输出 | 决策 |
|---|---|---|---|---|
| 自制全景上传 | 用户自制 2:1 equirectangular 图片或视频 | 上传、校验比例、绑定节点、播放器预览 | `AssetManifestItem` panorama asset | 通过则开放人工全景节点；不通过则按平面资产处理 |
| 全景图 spike | 2-3 个 provider、同一批 prompt、2:1 样图 | 静态检查 + Three.js 球面加载 + 桌面/手机人工检查 | `PanoramaSpikeReport` | 是否开放全景节点；不影响平面图默认路径 |
| 发布架构 spike | 同一份 `player-manifest.json` | build-per-game 与 runtime-manifest 各跑一遍 | `DeploymentArchitectureSpikeReport` | v1 默认发布架构 |
| 播放器交互 spike | 运行端 demo、音频、TTS、陀螺仪 | 真实浏览器矩阵验证 | `PlayerInteractionSpikeReport` | 开始手势、TTS 时序、陀螺仪降级策略 |
...
1. 先做 Phase -1：用现有 demo 或 3 节点 fixture 生成最小 `player-manifest.json`，明确写 `visualMode = "flat"`、`projection = "planar"`，让 `packages/panorama-player` 读取 manifest、本地试玩、部署 Vercel，拿到真实可玩 URL；第一个 demo 不验收全景。
2. 平面 demo 整体功能没问题后，再开放“自制全景上传”测试：用户上传 2:1 equirectangular 图片或视频，系统校验比例、绑定为 panorama 资产并播放，不调用生成 API。
3. 再做 Phase -0.5：全景图技术 spike。这里不是把图片拼成 3D 模型，而是把候选 2:1 图当作 equirectangular 球面内侧纹理加载，检查接缝、极点、人物形变、移动端观感；不通过就继续保持平面图默认模式。
4. 同时做播放器发布架构 spike：用同一份 manifest 对比 build-per-game 和 runtime-manifest，决定 v1 以后是每个游戏单独 build，还是播放器部署一次后按 `gameId` 拉 manifest。
5. 同时做播放器交互 spike：验证“开始体验”手势、音频解锁、TTS 不叠加、iOS 陀螺仪授权和拖拽降级。
6. spike 通过后进入 v1：新增 `src/lib/creation-flow/types.ts`、`node-builder.ts`、`use-creation-flow-store.ts`，做 Step 1-6、节点画布、节点资产面板和 `.dfstory` export。
7. 做 `packages/panorama-player` 的 manifest 泛化，保留现有 Three.js 渲染器，但从 generated manifest 读取节点、角色、变量和资产。
8. 做 MVP bundled asset 模式：轻量图片、音频、上传的小视频复制进构建目录，随 Vercel 包部署；大资产和生产对象存储后置。
9. 接入 provider adapter：DeepSeek 用于 Step 1-6，Image 2 先用于平面图生成；只有全景 spike 通过后才开放批量全景图生成，MiniMax 用于 TTS，视频 API 继续待定，demo 阶段只上传视频。
10. 批量生成前接入 `GenerationBudgetEstimate`，显示项目级总成本、节点数、对白数和预算上限。
11. 发布前接入 `SchemaConsistencyReport`，校验 `.dfstory`、asset-manifest、player-manifest 的节点、资产、跳转和语音引用。
12. 建立服务端 DB 落点：deploy records、playtest events、feedback、evolution reports。
13. 接入 packager、platform-hosted Vercel 默认目标和 `local-codex-worker` 部署执行器；正式 MVP 再迁移到独立 Node Worker + queue。
14. 最后再接匿名 playtest events、通关反馈、自进化报告和候选改版建议；用户自有 Vercel target、OAuth、自定义域名、多平台部署、生产对象存储放到商业化阶段。

这条路径的关键是先证明“播放器模板 + flat manifest + Vercel URL”成立，再验证自制全景上传、全景图生成、发布架构和移动端交互这些高风险点。确认平面底座可用以后，再投入完整前六步和节点画布；全景能力作为增强模式插入，不拖慢第一阶段。
